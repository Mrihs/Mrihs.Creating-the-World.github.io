...
Beim Skulpturieren werden jeweils die Vertices innerhalb des Meshes entsprechend des Effektes verschoben. Dies bedeutet allerdings auch, dass die Topografie des Meshes genug Vertices beinhalten muss, damit diese bearbeitet werden können. 
Wird etwa einfach der Standardwürfel verwendet, können lediglich dessen acht Ecken bearbeitet werden. Es ist allerdings auch möglich, in der Sidebar unter dem Reiter «*Tools*» die Option «*Dyntopo*» auszuwählen. Diese Option ermöglicht es, dass beim Anwenden der Pinsel auf das Mesh direkt neue Vertices erstellt werden, sodass eine dynamische Bearbeitung der Topografie des Meshes möglich ist. Dies hat allerdings 
zur Folge, dass Mesh-Daten, wie beispielsweise die UVs, verformt werden.
...
![Eine Plane zur Vorbereitung des Retopology-Prozesses.](Images/Chapter_41/2_Plane_Modified.png){#fig-1_2 fig-align="left"}
...
![Eine Plane zur Vorbereitung des Retopology-Prozesses.](Images/Chapter_41/3_Mesh_Extrusion.png){#fig-1_3 fig-align="left"}

![Eine Plane zur Vorbereitung des Retopology-Prozesses.](Images/Chapter_41/4_Mesh_Head.png){#fig-1_4 fig-align="left"}
...
![Eine Plane zur Vorbereitung des Retopology-Prozesses.](Images/Chapter_41/6_Object_Head.png){#fig-1_5 fig-align="left"}
...
sollte beim Retopology darauf geachtet werden, dass Körperbereiche, welche sich stärker verformen (beispielweise, Ellbogen, Knie, Füsse, Hände, Schultern, Gesicht), etwas mehr Vertices aufweisen als Körperteile, welche weniger stark verformbar sein sollten.
...
Eine etwas einfachere Option zur Erstellung von Charakteren ergibt sich zudem durch die Verwendung des Skin-Modifiers. Hierfür wird ein neues Objekt erstellt und alle dessen Vertices in deren Mittelpunkt verbunden, indem im Edit-Mode mit der Taste alle Vertices ausgewählt werden und mit der Taste {{< kbd A >}} das Merge-Menü geöffnet wird. Darin kann anschliessend die Option «*Merge | At 
Center*» ausgewählt werden. Anschliessend kann dem Objekt ein Mirror-Modifier, gefolgt von einem Skin-Modifier, hinzugefügt werden. 
